...
func CreateMessage(message *Message) error {
	if result := GetDB().Create(&message); result.Error != nil {
		return result.Error
	}
	return nil
}
...
func QueryMessageList(date *string, fromUserId int64, ToUserId int64) ([]*Message, error) {
	...
	result := GetDB().Where("( (from_user_id = ? and to_user_id = ?) or (from_user_id = ? and to_user_id = ?) ) and created_at > ?", fromUserId, ToUserId, ToUserId, fromUserId, date).Order("created_at asc").Find(&MessageList)
	if result.Error != nil {
