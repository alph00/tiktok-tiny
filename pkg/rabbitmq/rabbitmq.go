...
func NewRabbitMQSimple(queueName string, autoAck bool) *RabbitMQ {
	// 创建RabbitMQ实例
	rabbitmq := NewRabbitMQ(queueName, "", "", config.GetInt("server.prefetchCount"))
	var err error
	// 获取connection
...
func (r *RabbitMQ) PublishSimple(ctx context.Context, message []byte) error {
	...
	if err != nil {
		...
		// log.Errorf("MQ 生产者错误：%v", err.Error())
		if r.conn.IsClosed() || r.channel.IsClosed() {
			log.Print("RabbitMQ 连接断开，需要重连")
...
func (r *RabbitMQ) ConsumeSimple() (<-chan amqp.Delivery, error) {
	...
	if err != nil {
		log.Printf("MQ 消费者错误：%v", err.Error())
		return nil, err
	}
	...
	msgs, err := r.channel.Consume(
		...
		"", // consumer
		// 是否自动应答
		config.GetBool("consumer.favorite.autoAck"), // auto-ack
		// 是否独有
		false, // exclusive
