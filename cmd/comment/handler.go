...
func (s *CommentServiceImpl) CommentAction(ctx context.Context, req *comment.CommentActionRequest) (resp *comment.CommentActionResponse, err error) {
	...
	//TODO  根据视频id获取视频信息
	video, err := model.GetVideoById(ctx, req.VideoId)
	if err != nil {
		res := &comment.CommentActionResponse{
			StatusMsg:  "该视d对应的视频不存在",
			StatusCode: -1,
		}
		return res, nil
	}
	...
	if actionType == 1 {
		...
		if resComment == nil {
			...
			//TODO 获取该评论对应的视频信息
			videoByComment, err := model.GetVideoById(ctx, int64(resComment.VideoID))
			if err != nil {
				res := &comment.CommentActionResponse{
					StatusMsg:  "系统异常，评论无法删除",
					StatusCode: -1,
				}
				return res, nil
			}
...
func (s *CommentServiceImpl) CommentList(ctx context.Context, req *comment.CommentListRequest) (resp *comment.CommentListResponse, err error) {
	...
	for _, c := range commentList {
		...
		flolow, err := model.GetRelationByUserIDs(ctx, userId, int64(user.ID))
		if err != nil {
			res := &comment.CommentListResponse{
				StatusMsg:  "服务异常,获取用户关联信息失败",
				StatusCode: -1,
			}
			return res, nil
		}
		//TODO 从minio中获取头像URL
		avatar, err := minio.GetFileTemporaryURL(minio.AvatarBucketName, user.Avatar)
		if err != nil {
			res := &comment.CommentListResponse{
		...
		backgroundUrl, err := minio.GetFileTemporaryURL(minio.BackgroundImageBucketName, user.BackgroundImage)
		if err != nil {
			res := &comment.CommentListResponse{
